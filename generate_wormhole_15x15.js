...
async function apply15x15Wormhole() {
  ...
  // Aplicar la regla 15x15
  const sections = [];
  for (let i = 0; i < 15; i++) {
    const data = wormhole15x15[i];
    sections.push({
      id: `er_sec_${i}`,
      title: `Sección ${i + 1}: ${data.title}`,
      text: data.text,
      // Usar loremflickr temporalmente o fotos estelares, 
      // mejor usamos fotos espaciales variadas con lock
      image: `https://loremflickr.com/1200/800/space,galaxy,nebula?lock=${5000 + i}`,
      style: i % 3 === 0 ? "highlight" : "normal"
    });
  }

  // Inject a video in the first section (from course content or a known drive url)
  sections[0].video = 'https://drive.google.com/file/d/1vKWif4d_wiUTec2o-UJQiZGaHq6S4GMM/preview'; 
  sections[0].image = null;
  ...
  console.log("Regla 15x15 aplicada a los Agujeros de Gusano con 150 líneas únicas de contenido avanzado.");
}
