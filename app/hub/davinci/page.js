...
function ConstellationLines() {
  return (
    <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 5 }}>
      {DAVINCI_MODULES.map((mod, i) => {
        if (i === DAVINCI_MODULES.length - 1) return null;
        const nextMod = DAVINCI_MODULES[i + 1];
        return (
          <line
            key={i}
            x1={mod.coords.left}
      ...
      <defs>
        <linearGradient id="davinci-grad" x1="0%" y1="0%" x2="100%" y2="0%">
...
// ─── Nodo de Módulo ────────────────────────────────────────────────────────
function DaVinciModuleNode({ mod, idx, isCompleted, isPlayable }) {
...
// ─── HUB Principal ──────────────────────────────────────────────────────────
export default function DaVinciHub() {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#050a12', overflow: 'hidden' }}>
      ...
      <main style={{
          ...
          >
            <div style={{
              ...
              <h1 style={{
                ...
                margin: 0,
                fontSize: '2rem',
                textShadow: '0 0 10px rgba(212, 168, 67, 0.6), 0 0 20px rgba(212, 168, 67, 0.4)',
                letterSpacing: '2px',
                fontFamily: 'var(--font-heading)',
              ...
              <p style={{
                ...
                El Genio Universal del Renacimiento · 5 Módulos
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <div style={{
          ...
          backdropFilter: 'blur(10px)',
          zIndex: 200,
          boxShadow: '0 0 30px rgba(0,0,0,0.8), inset 0 0 20px rgba(212, 168, 67, 0.1)',
          ...
          <div style={{
            ...
            {completedIds.filter(id => id.startsWith('davinci_m')).length} / {DAVINCI_MODULES.length} módulos
          </div>
        </div>
      </main>
    </div>
