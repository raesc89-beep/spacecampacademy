...
const searchCommons = (query) => {
  return new Promise((resolve) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'SpaceCampBot/4.0 (raesc89@spacecamp.com)' } }, (res) => {
      let body = '';
...
async function fixImages() {
  console.log('Descargando imágenes reales de Wikimedia Commons...');
  const urls1 = await searchCommons('Mars surface');
  const urls2 = await searchCommons('Mars rover');
  
  // Combina y quita duplicados
  const allUrls = [...new Set([...urls1, ...urls2])];
  console.log(`Encontradas ${allUrls.length} imágenes 100% reales de Marte.`);
  ...
  // Reemplazar loremflickr en todos los robots
  jsData.forEach(course => {
    if (course.id.startsWith('robots_') && course.contentEs && course.contentEs.sections) {
  ...
  console.log('Imágenes de gatos purgadas. Se inyectaron fotografías de Marte 100% reales en las 120 secciones.');
}
